...
ReviewSchema.statics.calculateAverageRating = async function (productId) {
  const result = await this.aggregate([ //aggregate：DB的聚合方法(多條件搜尋、計算)
...
ReviewSchema.post("save", async function () { //只有schema.save()時觸發
  // console.log('post save hook called');
  await this.constructor.calculateAverageRating(this.product); //呼叫自身靜態方法
});

ReviewSchema.post("remove", async function () { //只有schema.remove()時觸發
